#Create a linear model to run VIF analysis
linearmodel <- lm(SalePrice~.,data = Housing)
summary(linearmodel)
...
#Take attributes with VIF less than 10
attributes <- sorttable[sorttable < 10]
attributes

#Creating a vector of attributes with VIF less than 10
vif10 <- c("GarageCars", "GarageArea", "TotRmsAbvGrd" ,"TotalBsmtSF", "YearBuilt", "OverallQual", "FullBath", "BsmtFin", "Bedroom",
...
lmOut <- lm(SalePrice ~ GarageCars + GarageArea + TotRmsAbvGrd + TotalBsmtSF + YearBuilt + OverallQual + FullBath + BsmtFin + Bedroom +
              HalfBath + BsmtFullBath + Fireplaces + LotFrontage + Kitchen + OverallCond + LotArea +
              WoodDeckSF + BsmtHalfBath + PoolArea, data = Housing)

summary(lmOut)

#Create new dataset with shortlisted attributes
Housing <- Housing[,c("SalePrice",vif10)]
